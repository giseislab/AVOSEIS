...
my $VALVEURL = $VALVEJSP."?a=rawData&o=csv&tz=GMT&src.0=avo_seismic_hypocenters&st.0=$STARTTIME&et.0=$ENDTIME&north.0=$MAXLAT&south.0=$MINLAT&east.0=$MAXLON&west.0=$MINLON&rk.0=1";
print $VALVEURL."\n";
...
if ($#typelines==0) { # there should only be 1 url line! It looks like    <url>url_to_csv_file</url>
	my $type = $typelines[0];
	...
	die(get($VALVEURL)) unless ($type eq "rawData");
}
...
if ($#lines==0) { # there should only be 1 url line! It looks like    <url>url_to_csv_file</url>
	...
	die(($#lines+1)." lines found with a url tag. Aborting.\n");
}
