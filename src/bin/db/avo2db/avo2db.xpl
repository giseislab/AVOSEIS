...
# by M. West initially 4/24/2006. The goal of avo2db is to provide 
# a consistent routine for converting avo-style pickfiles into Datascope 
# compatible database tables. This development was spurred by the 
# fact that pick2db was not creating three letter chan names in the 
# resulting arrival tables and id numbers were duplicated in ecah month.
# 
...
$Usage = "\nUsage: $0 [-v] [-r reference_db[:reference_db...]] pickfile [pickfile...] dbname\n\nSee pick2db man page for detailed use. AVO2DB is a custom version specific to AVO pickfiles. It is intended only for in house use. Primary differences include (1) AVO2DB creates real channel names when used with suitable pick files. Hence, the -c and -p flags have been disabled. Three letter channel names have been in pickfiles since mid-1997. (2) AVO2DB assigns id numbers that are based on the month the data was recorded. Each month has unique id numbers which allows month databases to be concatonated together without renumbering. This entire numbering scheme relies on databases being created one month at a time. This is reasonable however as this is how AVO stores pickfiles. (3) A remark table is produced by AVO2DB. When joined with the origin table (using the commid), each row will have a remark which includes the pickfile name from which the data is derived. (4) Error ellipse information is generated by project_ellipse_avo instead of project_ellipse. For reasons unknown, project_ellipse no longer appears to return error infor consistent with the pickfile (or consistent with its own man page?!). project_ellipse_avo is a modest revision by M. Thorne that appears to correctly handle error info. Note that the project_ellipse output has not been rigorously tested. However, the results of project_ellipse are clearly incorrect.\n\n";
